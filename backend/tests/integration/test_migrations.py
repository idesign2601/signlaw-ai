...
class TestConstraintsAreEnforced:
    async def test_duplicate_document_hash_is_rejected(self, session: AsyncSession) -> None:
        ...
        await session.execute(insert, {"id": uuid.uuid4(), "sha": sha})
        await session.flush()

        # Core execute() emits the statement immediately, so the unique
        # violation surfaces here and never reaches flush().
