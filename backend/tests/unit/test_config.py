...
class TestEmbeddingSettings:
    ...
    def test_unknown_model_skips_dimension_check(self) -> None:
        settings = EmbeddingSettings(model="acme/custom-embedder", dimensions=999)
        assert settings.dimensions == 999
...
class TestVectorStoreSettings:
    def test_collection_name_is_versioned(self) -> None:
        vector = VectorStoreSettings(collection_prefix="signlaw", index_version=3)
        assert vector.collection_name == "signlaw_v3"

    def test_collection_name_for_arbitrary_version(self) -> None:
        # Blue/green re-indexing builds vN+1 alongside the live vN.
        vector = VectorStoreSettings(index_version=1)
        assert vector.collection_name_for(2) == "signlaw_v2"

    def test_version_must_be_positive(self) -> None:
        with pytest.raises(ValueError, match="index version must be >= 1"):
            VectorStoreSettings().collection_name_for(0)
