...
@pytest.fixture(scope="module")
async def report(service, suite):  # noqa: ANN001, ANN201
    """Run the suite once and share the result across assertions."""
    return await EvalRunner(service=service).run(suite)


class TestReleaseGate:
    def test_no_citation_rests_on_outdated_text(self, report) -> None:  # noqa: ANN001
    ...
    def test_pass_rate(self, report) -> None:  # noqa: ANN001
        assert report.pass_rate >= MIN_PASS_RATE, report.render()

    def test_retrieval_accuracy(self, report) -> None:  # noqa: ANN001
    ...
    def test_citation_accuracy(self, report) -> None:  # noqa: ANN001
        assert report.citation_accuracy >= MIN_CITATION_ACCURACY, report.render()

    def test_behaviour_accuracy(self, report) -> None:  # noqa: ANN001
        # Declining when it should, asking when it should.
        assert report.behaviour_accuracy >= MIN_BEHAVIOUR_ACCURACY, report.render()

    def test_no_errors(self, report) -> None:  # noqa: ANN001
        assert report.errors == 0, report.render()


class TestCalibration:
    def test_high_confidence_outperforms_low(self, report) -> None:  # noqa: ANN001
...
class TestBehaviour:
    async def test_ambiguous_municipality_asks(self, service) -> None:  # noqa: ANN001
    ...
    async def test_out_of_scope_declines(self, service) -> None:  # noqa: ANN001
        from app.services.rag_service import AnswerOutcome

        result = await service.answer("What is the population of Surrey?")
        assert result.outcome is AnswerOutcome.OUT_OF_SCOPE

    async def test_unknown_sign_type_abstains(self, service) -> None:  # noqa: ANN001
