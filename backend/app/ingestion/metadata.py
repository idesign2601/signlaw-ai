...
# The number must be delimited and start with a non-zero digit. Without the
# delimiter, "scan001.pdf" yielded bylaw number "001" — a fabricated citation
# from a filename that carries no bylaw number at all.
_BYLAW_NUMBER_IN_FILENAME = re.compile(
    r"(?:^|[_\s-])(?:by-?law[_\s-]*)?(?:no[._\s-]*)?"
    r"(?P<number>[1-9]\d{2,4}(?:[-–]\d{2,4})?)(?=[_\s.-]|$)",
    re.IGNORECASE,
)
