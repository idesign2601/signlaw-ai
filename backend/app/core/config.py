...
#
# Path defaults are anchored here rather than left relative to the working
# directory, because commands run from both the repository root (scripts) and
# from backend/ (the Makefile), and a relative default silently means two
# different directories depending on which one invoked it.
...
class IngestionSettings(BaseModel):
    ...
    # Anchored to the repository root for the same reason as tessdata_dir below.
    corpus_dir: Path = _REPO_ROOT / "data" / "corpus"
    blob_dir: Path = _REPO_ROOT / "data" / "blobs"
    ...
    # Anchored to the repository root rather than left relative: the Makefile
    # runs every command from backend/, so a relative default resolved to
    # backend/data/models while `make fetch-models` wrote to <root>/data/models.
    # The two silently disagreed and the weights were downloaded twice.
    # Docker overrides this with INGESTION__TESSDATA_DIR=/models/tessdata.
    tessdata_dir: Path = _REPO_ROOT / "data" / "models" / "tessdata"
