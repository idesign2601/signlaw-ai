...
class RerankerProtocol(Protocol):
    ...
    A cross-encoder scores the query and passage together rather than embedding
    them independently, which is markedly more accurate but too slow to run over
    the whole corpus — hence retrieve broadly, then rerank narrowly.

    A structural Protocol, not a base class: implementations live in
    ``app.adapters.reranker`` and must not have to import from ``app.rag``.
    """
...
class HybridRetriever:
    ...
    async def _sparse_search(self, query: str, filters: RetrievalFilters) -> list[RetrievedChunk]:
        ...
        except Exception as exc:  # a malformed tsquery must not 500
...
def _row_to_chunk(
    row: Any, *, dense_score: float | None = None, sparse_score: float | None = None
) -> RetrievedChunk:
    """Map a result row onto the citation-carrying result object.

    ``row`` is a SQLAlchemy ``Row`` whose columns are known only from the SQL
    above, so it is typed ``Any`` rather than pretending to a static shape.
    """
    coordinates = None
