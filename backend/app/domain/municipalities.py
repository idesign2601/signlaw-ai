...
_INCORPORATION_PREFIX = re.compile(
...
_PROVINCE_SUFFIX = re.compile(r",?\s*\b(?:b\.?c\.?|british\s+columbia|canada)\b\.?$")


def _fold(value: str) -> str:
    """Lower-case, strip accents, drop a trailing province suffix."""
    decomposed = unicodedata.normalize("NFKD", value)
    ascii_only = "".join(char for char in decomposed if not unicodedata.combining(char))
    lowered = ascii_only.casefold().strip()
    return _PROVINCE_SUFFIX.sub("", lowered)


def qualify(value: str) -> str:
    """Normalise a name while **keeping** the incorporation prefix.

    "City of Langley" becomes ``city-of-langley`` and "Township of Langley"
    becomes ``township-of-langley``. This is what makes the two Langleys
    distinguishable: :func:`slugify` deliberately removes the prefix, so every
    qualified form of an ambiguous name collapses to the same bare key.
    """
    return re.sub(r"[^a-z0-9]+", "-", _fold(value)).strip("-")


def slugify(value: str) -> str:
    """Normalise a name to a bare comparison key.
    ...
    """
    stripped = _INCORPORATION_PREFIX.sub("", _fold(value))
...
class MunicipalityRegistry:
    ...
    def __post_init__(self) -> None:
        ...
        for record in self.records:
            # Qualified keys keep the incorporation prefix, so "city-of-langley"
            # and "township-of-langley" stay distinct. Bare keys drop it.
            keys = {
                record.slug,
                qualify(record.official_name),
                slugify(record.name),
                slugify(record.official_name),
            }
            for alias in record.aliases:
                keys.add(slugify(alias))
                keys.add(qualify(alias))

            if record.ambiguous_bare_name:
                # Only the qualified forms and the explicit slug resolve; the
                # bare name is removed below so it can never silently pick one.
        ...
        # Remove bare ambiguous keys after every record is registered, so one
        # municipality cannot claim the key before its twin is seen.
    ...
    def resolve(self, value: str) -> MunicipalityRecord | None:
        ...
        if not value or not value.strip():
            return None
        # Qualified form first: "City of Langley" must not fall through to the
        # bare "langley" lookup, which is deliberately unresolvable.
        return self._by_key.get(qualify(value)) or self._by_key.get(slugify(value))
    ...
    def candidates(self, value: str) -> tuple[MunicipalityRecord, ...]:
        ...
        """
        qualified = self._by_key.get(qualify(value))
    ...
    def is_ambiguous(self, value: str) -> bool:
        # A qualified name is never ambiguous, however ambiguous its bare form.
        if qualify(value) in self._by_key:
            return False
    ...
    def find_in_text(self, text: str) -> tuple[MunicipalityRecord, ...]:
        ...
        """
        # `qualify`, not `slugify`: the haystack must keep "district of" so that
        # a qualified key can match it.
        haystack = qualify(text)
        ...
        for key in sorted(self._by_key, key=len, reverse=True):
            ...
            match = re.search(rf"(?:^|-){re.escape(key)}(?:-|$)", haystack)
            if match is None:
                continue
            ...
            # Blank out the matched span so a shorter name nested inside it
            # cannot also match. Without this, "north vancouver" is found and
            # then "vancouver" is found again inside the same words.
