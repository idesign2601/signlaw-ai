...
# Vocabulary that indicates a genuine sign-bylaw question.
_DOMAIN_TERMS = re.compile(
    r"\b(sign|signage|signs|fascia|awning|canopy|banner|billboard|marquee|"
    r"projecting|freestanding|pylon|monument|sandwich board|a-?frame|"
    r"portable|window graphic|wall mural|illuminat\w*|digital display|"
    r"bylaw|by-?law|permit|zoning|zone|setback|frontage|facade|façade|"
    r"height|area|dimension|temporary|real estate sign|construction sign)\b",
    re.IGNORECASE,
)
...
class QueryRouter:
    ...
    def _is_out_of_scope(query: str, municipalities: tuple[MunicipalityRecord, ...]) -> bool:
        ...
        if _OUT_OF_SCOPE_TERMS.search(query):
            return True
        if _DOMAIN_TERMS.search(query):
            return False
        # No domain vocabulary at all. A bare city name is not a question this
        # corpus can answer.
        return True
