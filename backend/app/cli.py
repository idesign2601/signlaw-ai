...
GREEN = lambda text: _c("32", text)  # noqa: E731
YELLOW = lambda text: _c("33", text)  # noqa: E731
RED = lambda text: _c("31", text)  # noqa: E731
CYAN = lambda text: _c("36", text)  # noqa: E731
...
async def _with_session(settings: Settings, coroutine):  # noqa: ANN001, ANN202
    """Run a coroutine with a database session, disposing the engine after."""
    ...
    try:
        async with factory() as session:
            return await coroutine(session, engine)
    finally:
...
async def _run_health(settings: Settings, as_json: bool) -> int:
    ...
    from app.services.health_service import HealthService, render_report
...
def _answer_as_dict(result) -> dict[str, object]:  # noqa: ANN001
    return {
...
def _print_answer(result, *, show_trace: bool) -> None:  # noqa: ANN001
...
async def _active_collection_name(session) -> str:  # noqa: ANN001
    from sqlalchemy import text
