...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Check constraint names are given bare, without the `ck_<table>_`
        # prefix. Alembic builds these against the target metadata, whose
        # naming convention is `ck_%(table_name)s_%(constraint_name)s`, so the
        # prefix is added here and passing it in as well would produce
        # `ck_document_ck_document_...` in the database — a name the ORM models
        # would then never match. This applies to every CheckConstraint below.
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.CheckConstraint("total_documents >= 0", name="total_documents_non_negative"),
    )
