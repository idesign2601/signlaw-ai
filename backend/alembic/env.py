...
# Expression indexes (the full-text GIN index on chunk.body) cannot be reflected
# back into a comparable form, so autogenerate would propose dropping and
# recreating them on every run. They are excluded from comparison and managed
# explicitly in migrations instead.
UNCOMPARABLE_INDEXES = frozenset({"ix_chunk_body_fts"})
